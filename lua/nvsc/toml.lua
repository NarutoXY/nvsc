...
TOML.multistep_parser = function(options)
  ...
  local function matchDate()
    local year, month, day, n = getData(0, 10):match("^(%d%d%d%d)%-([0-1][0-9])%-([0-3][0-9])()")
  ...
  local function matchTime()
    local hour, minute, second, n = getData(0, 19):match("^([0-2][0-9])%:([0-6][0-9])%:(%d+%.?%d*)()")
  ...
  local function matchTimezone()
    local eastwest, offset, zero, n = getData(0, 6):match("^([%+%-])([0-9][0-9])%:([0-9][0-9])()")

    if not eastwest then
      return nil
    end
    step(n - 1)
